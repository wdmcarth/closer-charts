...
## Consume the data from other apps (read-only API)

A flat per-pitcher JSON snapshot is auto-published at:

```
https://<your-user>.github.io/closer-charts/data/api.json
```

No auth required. Auto-regenerated by a GitHub Action whenever the editor
saves or a refresh-* workflow commits, so the file stays in sync with the
chart within ~30–60s of any change.
...
{
  "schemaVersion": 1,
  "generatedAt": "2026-05-16T20:36:11+00:00",
  "pitcherCount": 174,
  "pitchers": [
    {
      "league": "AL",
      ...
      "team_abbr": "BAL",
      "team_id": 110,
      "team_notes": "**Ryan Helsley (elbow) was placed on the 15-day IL (4.29)",
      "role": "closer",
      ...
      "on_40_man": true,
      "hand": "R",
      "blue": true, "orange": false, "yellow": false,
      "magenta": false, "green": false,
      "usage_tag": null,
      "injury_tag": null,
      "other_tag": null,
      "last_game": "2026-05-11"
    },
    …
  ]
}
...
|---|---|---|
| `league` | `"AL"` / `"NL"` | |
| `lev` | 1–5 / null | The team's LEVCON rating. |
| `team` | string | Full team name. |
| `team_abbr` | string / null | e.g. `"BAL"`. Sourced from MLB rosters. |
| `team_id` | int / null | MLB Stats API team id. |
| `team_notes` | string / null | The right-hand injury/availability column on the chart. |
| `role` | `"closer"` / `"stopper"` / `"stealth"` / `"upside"` / `"hybrid"` | |
| `role_label` | string | Human-readable role name. |
| `role_position` | int | 1 = primary, 2 = secondary, etc. (preserves committee order). |
| `player` | string | The chip's display name. |
| `mlbamid` | int / null | MLB Stats API player id. `null` for chips not bound to a roster (prospects without 40-man entries can show this). |
| `level` | `"MLB"` / `"AAA"` / `"AA"` / `"A+"` / `"A"` / `"ROK"` / ... / null | From the org pitcher index. |
| `on_40_man` | bool / null | `true` iff `level === "MLB"`. |
| `hand` | `"L"` / `"R"` / null | Pitch hand. |
| `blue` / `orange` / `yellow` / `magenta` / `green` | bool | The effective color flags. **Magenta auto-fires when usage_tag is non-empty** unless the user has explicitly dismissed it (so a consumer can treat Magenta as a workload signal too). |
| `usage_tag` | string / null | Auto-derived from recent game log. Comma-joined list, e.g. `"b2b, 3-of-4"` or `"1.2 IP, 31 np"`. |
| `injury_tag` | string / null | Manual editor field (e.g. `"IL"`, `"paternity"`, `"suspended"`). |
| `other_tag` | string / null | Manual free-text (e.g. `"HLR"`, `"FROOP"`). |
| `last_game` | `"YYYY-MM-DD"` / null | Date of the pitcher's most recent appearance. |

Worked example (Python):

```python
import requests
data = requests.get("https://wdmcarth.github.io/closer-charts/data/api.json").json()
# All AL closers that auto-magenta'd this week (workload concern)
hot_closers = [p for p in data["pitchers"]
               if p["league"] == "AL"
               and p["role"] == "closer"
               and p["role_position"] == 1
               and p["magenta"]]
for p in hot_closers:
    print(f'{p["team_abbr"]:>3} {p["player"]:<20} {p["usage_tag"]}')
```

To regenerate manually (e.g. after editing `build_api.py`): GitHub repo →
**Actions** → **Build api.json** → **Run workflow**.
